...
param Bus_Pd		{n in BUS, t in LOAD, s in SCN};  # Creates the hourly load per bus
...
subject to PowerBal{t in LOAD, s in SCN, k in BUS}: # Node Balance Constraint, steady-state
	...
	+ sum{g in GEND: genD_bus[g]==k}gen_supply[g,t,s] - Bus_Pd[k,t,s] = 0; # supply and load at bus ##############
																	# scn net load: Bus_pd - PgWndFrm[wf[w],s]=0;
